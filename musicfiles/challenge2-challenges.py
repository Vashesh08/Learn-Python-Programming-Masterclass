...
loc = 1
exit_to_destination = [(item, locations[item]) for item, value in exits.items() if loc in value.values()]
print(exit_to_destination)
for loc in sorted(locations):
    exit_to_destination = [(item, locations[item]) for item, value in exits.items() if loc in value.values()]
    print("Locations leading to {}".format(loc), exit_to_destination, sep="-")
