...
db.execute("CREATE TABLE IF NOT EXISTS history "
           "(time2 TEXT NOT NULL, time TIMESTAMP NOT NULL, account TEXT NOT NULL,"
...
class Account(object):
    ...
    @staticmethod
    def _current_time_info():
        # print(type(pytz.datetime.datetime.astimezone(Account._current_time()).tzname()))
        return pytz.datetime.datetime.astimezone(Account._current_time()).tzname()
    ...
    def _save_update(self, amount):
        new_balance = self._balance + amount
        deposit_time = Account._current_time()
        deposit_time_info = Account._current_time_info()
        db.execute("UPDATE accounts SET balance = ? WHERE name = ?", (new_balance, self.name))
        db.execute("INSERT INTO history VALUES(?, ?, ?, ?)", (deposit_time_info, deposit_time, self.name, amount))
        db.commit()
        self._balance = new_balance
